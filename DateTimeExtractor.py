# -*- coding: utf-8 -*-
# import nltk
# from nltk.tag.stanford import StanfordNERTagger as stNer
#
# class dateTimeUnit:
...
try:
    ...
    print """Requires eGenix.com mx Base Distribution : http://www.egenix.com/products/python/mxBase/"""
...
class dateTimeUnit:
    def __init__(self, inputText):
        self.text = inputText
        # Predefined strings.
        self.numbers = "(^a(?=\s)|one|two|three|four|five|six|seven|eight|nine|ten| \
                  eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen| \
                  eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty| \
                  ninety|hundred|thousand)"
        self.day = "(monday|tuesday|wednesday|thursday|friday|saturday|sunday)"
        self.week_day = "(monday|tuesday|wednesday|thursday|friday|saturday|sunday)"
        self.month = "(january|february|march|april|may|june|july|august|september| \
                  october|november|december)"
        self.dmy = "(year|day|week|month)"
        self.rel_day = "(today|yesterday|tomorrow|tonight|tonite)"
        self.exp1 = "(before|after|earlier|later|ago)"
        self.exp2 = "(this|next|last)"
        self.iso = "\d+[/-]\d+[/-]\d+ \d+:\d+:\d+\.\d+"
        self.year = "((?<=\s)\d{4}|^\d{4})"
        self.regxp1 = "((\d+|(" + self.numbers + "[-\s]?)+) " + self.dmy + "s? " + self.exp1 + ")"
        self.regxp2 = "(" + self.exp2 + " (" + self.dmy + "|" + self.week_day + "|" + self.month + "))"

        self.reg1 = re.compile(self.regxp1, re.IGNORECASE)
        self.reg2 = re.compile(self.regxp2, re.IGNORECASE)
        self.reg3 = re.compile(self.rel_day, re.IGNORECASE)
        self.reg4 = re.compile(self.iso)
        self.reg5 = re.compile(self.year)
    ...
    def tag(self):
        ...
        found = self.reg1.findall(self.text)
        found = [a[0] for a in found if len(a) > 1]
        for timex in found:
            timex_found.append(timex)

        # Variations of this thursday, next year, etc
        found = self.reg2.findall(self.text)
        found = [a[0] for a in found if len(a) > 1]
        for timex in found:
            timex_found.append(timex)

        # today, tomorrow, etc
        found = self.reg3.findall(self.text)
        for timex in found:
            timex_found.append(timex)

        # ISO
        found = self.reg4.findall(self.text)
        for timex in found:
            timex_found.append(timex)

        # Year
        found = self.reg5.findall(self.text)
        for timex in found:
            timex_found.append(timex)

        # Tag only temporal expressions which haven't been tagged.
        for timex in timex_found:
            self.text = re.sub(timex + '(?!</TIMEX2>)', '<TIMEX2>' + timex + '</TIMEX2>', self.text)
    ...
    def ground(self, tagged_text, base_date):
        ...
        for timex in timex_found:
            ...
            if re.search(self.numbers, timex, re.IGNORECASE):
                split_timex = re.split(r'\s(?=days?|months?|years?|weeks?)', \
                                       timex, re.IGNORECASE)
                value = split_timex[0]
                unit = split_timex[1]
                num_list = map(lambda s: self.hashnum(s), re.findall(self.numbers + '+', \
                                                                     value, re.IGNORECASE))
            ...
            elif re.match(r'last ' + self.week_day, timex, re.IGNORECASE):
                day = self.hashweekdays[timex.split()[1]]
                timex_val = str(base_date + RelativeDateTime(weeks=-1, \
                                                             weekday=(day, 0)))

            # Weekday in the current week.
            elif re.match(r'this ' + self.week_day, timex, re.IGNORECASE):
                day = self.hashweekdays[timex.split()[1]]
                timex_val = str(base_date + RelativeDateTime(weeks=0, \
                                                             weekday=(day, 0)))

            # Weekday in the following week.
            elif re.match(r'next ' + self.week_day, timex, re.IGNORECASE):
                day = self.hashweekdays[timex.split()[1]]
                timex_val = str(base_date + RelativeDateTime(weeks=+1, \
                                                             weekday=(day, 0)))
            ...
            # Month in the previous year.
                ...
                month = self.hashmonths[timex.split()[1]]
